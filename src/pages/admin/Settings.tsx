...
const PROMPT_PRESETS: Record<string, string> = {
  professional: `Kamu adalah asisten penjualan profesional untuk {{business_name}}. Gunakan bahasa formal dan sopan. Fokus pada closing penjualan dengan memberikan informasi produk yang lengkap dan akurat. Selalu gunakan data dari konteks berikut jika tersedia:\n\n{{context}}`,
...
export default function Settings() {
  ...
  const handleInviteAdmin = async () => {
    ...
    setInviting(true);
    const res = await supabase.functions.invoke("manage-admin", { body: { email: inviteEmail, password: invitePassword } });
    if (res.error || res.data?.error) {
      toast({ variant: "destructive", title: "Error", description: res.error?.message || res.data?.error });
    } else {
  ...
  const handleDeleteAdmin = async (userId: string) => {
    const res = await supabase.functions.invoke("manage-admin", { body: { user_id: userId }, method: "DELETE" as any });
    if (res.error || res.data?.error) {
      toast({ variant: "destructive", title: "Error", description: res.error?.message || res.data?.error });
    } else {
  ...
  const handlePresetChange = (preset: string) => {
    ...
    if (preset !== "custom" && PROMPT_PRESETS[preset]) {
      updateSetting("ai_system_prompt", JSON.stringify(PROMPT_PRESETS[preset]));
    }
  };
        ...
        <TabsContent value="admins">
          <div className="rounded-lg border border-border bg-card">
            <div className="flex items-center justify-between p-4 border-b border-border">
              ...
              <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
                <DialogTrigger asChild>
                  <Button size="sm" className="gap-2"><Plus className="h-4 w-4" /> Invite Admin</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>Invite Admin Baru</DialogTitle></DialogHeader>
                  <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="password" value={invitePassword} onChange={(e) => setInvitePassword(e.target.value)} placeholder="Minimal 6 karakter" />
                    </div>
                  </div>
                  <DialogFooter>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="py-2 px-3 text-xs">Email</TableHead>
                  <TableHead className="py-2 px-3 text-xs">Dibuat</TableHead>
              ...
              <TableBody>
                {admins.map((admin) => (
                  <TableRow key={admin.id}>
                    <TableCell className="py-2 px-3 text-sm font-medium">{admin.email}</TableCell>
                    <TableCell className="py-2 px-3 text-sm text-muted-foreground">{new Date(admin.created_at).toLocaleDateString("id-ID")}</TableCell>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>Admin "{admin.email}" akan kehilangan akses dashboard.</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Batal</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleDeleteAdmin(admin.user_id)}>Hapus</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </TableCell>
          ...
          <div className="rounded-lg border border-border bg-card p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={activePreset} onValueChange={handlePresetChange}>
                ...
                <SelectContent>
                  <SelectItem value="professional">Professional Seller</SelectItem>
                  <SelectItem value="friendly">Friendly Helper</SelectItem>
                  <SelectItem value="custom">Custom</SelectItem>
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">Pilih preset atau tulis prompt manual (otomatis switch ke Custom).</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) => handlePromptManualEdit(e.target.value)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="google/gemini-2.5-flash-lite">Gemini 2.5 Flash Lite (Cepat)</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <Input type="number" value={settings.ai_max_tokens || "1024"} onChange={(e) => updateSetting("ai_max_tokens", e.target.value)} min={100} max={8192} />
              </div>
            </div>

            <div className="space-y-2">
              <Label>Temperature: {settings.ai_temperature || "0.3"}</Label>
              <Slider value={[parseFloat(settings.ai_temperature || "0.3")]} onValueChange={([v]) => updateSetting("ai_temperature", String(v))} min={0} max={1} step={0.1} />
          ...
          <div className="rounded-lg border border-border bg-card p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input type="number" value={settings.default_daily_message_limit || "300"} onChange={(e) => updateSetting("default_daily_message_limit", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Default Quota Limit</Label>
                <Input type="number" value={settings.default_quota_limit || "1000"} onChange={(e) => updateSetting("default_quota_limit", e.target.value)} />
              </div>
            </div>
            <Separator />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input type="number" value={settings.anti_ban_delay_min || "2"} onChange={(e) => updateSetting("anti_ban_delay_min", e.target.value)} min={1} max={10} />
              </div>
              <div className="space-y-2">
